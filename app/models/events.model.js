...
exports.findByParams = async function (params) {
  ...
  let queryString = `SELECT event.id AS eventId, title,\n` +
  ...
  `COUNT(CASE WHEN event_attendees.attendance_status_id = 1 AND event_attendees.event_id=event.id THEN 1 ELSE NULL END) AS numAcceptedAttendees,\n` +
  `capacity\n` +
  `FROM event\n` +
  `INNER JOIN event_category ON event.id=event_category.event_id\n` +
  `INNER JOIN event_attendees ON event.id=event_attendees.event_id\n` +
  `INNER JOIN user ON event.organizer_id=user.id AND event_attendees.user_id=user.id`;
